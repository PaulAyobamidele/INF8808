...
export function simplifyDisplayTitles (data) {
  ...
  data.features.forEach(feature => {
    const type = feature.properties.TYPE_SITE_INTERVENTION
    if (!type) return
    if (type.startsWith('1.')) feature.properties.TYPE_SITE_INTERVENTION = 'Noyau villageois'
    else if (type.startsWith('2.')) feature.properties.TYPE_SITE_INTERVENTION = "Rue commerciale de quartier, d'ambiance ou de destination"
    else if (type.startsWith('3.')) feature.properties.TYPE_SITE_INTERVENTION = 'Rue transversale à une rue commerciale'
    else if (type.startsWith('4.')) feature.properties.TYPE_SITE_INTERVENTION = 'Rue bordant un bâtiment public ou institutionnel'
    else if (type.startsWith('5.')) feature.properties.TYPE_SITE_INTERVENTION = 'Rue en bordure ou entre deux parcs ou place publique'
    else if (type.startsWith('6.')) feature.properties.TYPE_SITE_INTERVENTION = 'Rue entre un parc et un bâtiment public ou institutionnel'
    else if (type.startsWith('7.')) feature.properties.TYPE_SITE_INTERVENTION = 'Passage entre rues résidentielles'
  })
}
