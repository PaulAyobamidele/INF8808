...
export function getTopPlayers (data) {
  ...
  const counts = {}
  data.forEach(row => {
    counts[row.Player] = (counts[row.Player] || 0) + 1
  })
...
export function summarizeLines (data) {
  ...
  const actMap = {}
  data.forEach(row => {
    if (!actMap[row.Act]) actMap[row.Act] = {}
    actMap[row.Act][row.Player] = (actMap[row.Act][row.Player] || 0) + 1
  })
  return Object.entries(actMap).map(([act, players]) => ({
    Act: +act,
...
export function replaceOthers (data, top) {
  ...
  return data.map(actData => {
    const topPlayers = actData.Players.filter(p => top.includes(p.Player))
    const otherCount = actData.Players
      .filter(p => !top.includes(p.Player))
      .reduce((sum, p) => sum + p.Count, 0)
    return {
      Act: actData.Act,
      Players: [...topPlayers, { Player: 'Other', Count: otherCount }]
    }
  })
}
